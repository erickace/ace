...
import { infoAtleta } from '../../modelos/listausuarios.interface';
...
@Component({
  selector: 'app-dashatletacouch',
  templateUrl: './dashatletacouch.component.html',
  styleUrls: ['./dashatletacouch.component.css']
})
export class DashatletacouchComponent implements OnInit {
  ...
  siFunciona() {
    ...
    if (this.contador < 1) {
      ...
      this.api.getHistorialGeneral(Number(localStorage.getItem('idAtleta'))).subscribe(data => {
        this.histoUser = data;
        var fechas: any = [];
    ...
    this.api.rcGET(objectVar__).subscribe(data => {
      ...
      if (this.datos_.length > 0) {
        ...
        function drawChart_() {
          ...
          var chart_ = new google.visualization.Gauge(document.getElementById('chart_divTC'));
          data_.setValue(0, 1, d1_);
          chart_.draw(data_, options_);
        }
    ...
    var d = new Date();
    this.fechaEnHistorial = String(d).split(" ", 3)[2] + " " + String(d).split(" ", 2)[1] + ". " + String(d).split(" ", 4)[3];
    this.fecha = d;
    setTimeout(() => {
      this.siFunciona()
    }, 1000);
  ...
  // solo funciona para couch -> omitir en dash atleta
  updateCount() {
